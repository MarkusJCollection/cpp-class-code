...
std::vector<int> sbox = // Hard-coded because my extended euclidean algorithm for polynomials wasn't working.
{
...
int substitute(int byte) {
	return sbox[byte];
}

int substitute(int byte, bool inverse) {
	if (inverse) {
...
void byteSub(std::vector<int>& aes_block) {
	for (int i = 0; i < 16; i++) {
		aes_block[i] = substitute(aes_block[i]);
	}
}

void inverseByteSub(std::vector<int>& aes_block) {
	for (int i = 0; i < 16; i++) {
		aes_block[i] = substitute(aes_block[i], true);
	}
}
